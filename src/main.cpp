// Adicionado para SDL_SetRelativeMouseMode
#include <SDL2/SDL.h>
// Adicionado para std::sqrt
#include <cmath> 
#include <cstdint>
#include <iostream>
#include <tuple>
...
enum class EntityMask : std::uint8_t { ENEMY, PLAYER, SOLID };
enum class PlaneType : std::uint8_t { NONE, WALL, FLOOR };


class Player : public Entity
{
  public:
    Player(Entity& entity, std::string name, std::string description);
    ~Player();
};
...
{
  public:
    Enemy(Entity& entity, std::string name, std::string description);
    ~Enemy();
};

class MapCell : public Component
{
  public:
    MapCell(Entity& entity, std::string name, std::string description);
    ~MapCell();
  private:
    PlaneType wallType = PlaneType::NONE;    

  public:
    operator =(const MapCell& rhs) = default;
    bool operator ==(const MapCell& rhs) const
    {
      return Component::operator==(rhs) && wallType == rhs.wallType;
    }
};
...
{
  public:
    Map(Entity& entity, std::string name, std::string description);
    ~Map();
  private:
  ...
  public:
    ...
    {
      return Component::operator==(rhs) && cells == rhs.cells && entities == rhs.entities;
    }
};
...
static void RenderPlayer(const Player &player, SDL_Renderer *renderer) {
  ...
  SDL_Rect playerRect = {static_cast<int>(player.position.x - playerSize / 2),
...
static void MovePlayer(const MapGrid &map, Player &player, float dirX,
                       float dirY, float deltaSeconds) {
  float newX = player.position.x + dirX * player.speed * deltaSeconds;
  float newY =
      player.position.y -
      dirY * player.speed * deltaSeconds; // Y invertido conforme sua lógica
...
static void Raycast(const MapGrid &map, Player &player,
...
int main(int argc, char *argv[]) {
  ...
  SDL_Window *window =
      SDL_CreateWindow("wolf3d", SDL_WINDOWPOS_CENTERED, SDL_WINDOWPOS_CENTERED,
  ...
  while (running) {
    ...
    if (keys[SDL_SCANCODE_W]) {
      MovePlayer(map, player, cos(player.angle), -sin(player.angle),
                 deltaSeconds);
    }
    ...
    if (keys[SDL_SCANCODE_A])
      MovePlayer(map, player, cos(player.angle - M_PI / 2),
                 -sin(player.angle - M_PI / 2), deltaSeconds);
